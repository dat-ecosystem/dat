...
authServer(port, (err, server, closeServer) => {
  if (err) throw err
  if (!server) return
  test('auth - whoami works when not logged in', t => {
  ...
  test('auth - publish before create fails',t => {
  ...
  test('auth - create dat to publish', t => {
    rimraf.sync(path.join(fixtures, '.dat'))
  ...
  test('auth - publish our awesome dat', t => {
  ...
  test('auth - publish our awesome dat with bad dat.json url', t => {
    fs.readFile(path.join(fixtures, 'dat.json'), (err, contents) => {
      t.ifError(err)
      const info = JSON.parse(contents)
      const oldUrl = info.url
      info.url = info.url.replace('e', 'a')
      fs.writeFile(path.join(fixtures, 'dat.json'), JSON.stringify(info), err => {
  ...
  test('auth - clone from registry', t => {
  ...
  test('auth - publish our awesome dat without a dat.json file', t => {
    rimraf(path.join(fixtures, 'dat.json'), err => {
      t.ifError(err)
  ...
  test('auth - bad clone from registry', t => {
  ...
  test('auth - logout prints correctly when trying to log out twice', t => {
    const cmd = dat + ' logout'
    const st = spawn(t, cmd, { cwd: baseTestDir })
    st.stderr.match(output => {
  ...
  test('auth - whoami works after logging out', t => {
    const cmd = dat + ' whoami'
    const st = spawn(t, cmd, { cwd: baseTestDir })
    st.stderr.match(output => {

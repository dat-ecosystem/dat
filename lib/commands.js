...
dat.push = function(options, cb) {
  ...
  request(target, {json: true}, function(err, resp, meta) {
    ...
    if (Object.keys(meta).indexOf('changes') === -1) return cb(new Error('no changes in metadata'))
    ...
    var post = request({ method: 'POST', uri: target + '/api/bulk', headers: headers })
    var serializer = through.obj(function write(obj, enc, next) {
      this.push(JSON.stringify(obj.data) + '\n')
      next()
    })
    
    readStream.pipe(serializer).pipe(post)
    
    if (options.verbose) {
      post.pipe(self.resultPrinter())
    } else if (!options.quiet) {
      post.pipe(ldj.parse()).pipe(self.progressLogStream('pushed'))
    }
    ...
    self._storage(options, function(err, store) {
      ...
      function pull() {
        self.meta.pullSchema(remote, function(err) {
          ...
          debug('pull', remote + '/api/changes')
          var pullStream = store.createPullStream(remote + '/api/changes', options)
          ...
          if (!options.quiet) writeStream.pipe(self.progressLogStream('pulled'))
...
dat.headers = function(options, cb) {
  ...
  var headers = ['id', 'version'].concat(this.meta.json.columns)
...
dat._storage = function(options, cb) {
  ...
  // custom blob backends must implement the same api as lib/blobs.js
  if (options.blobs) blobBackend = options.blobs
  self.blobs = blobBackend(paths.blobs, options.hasher)

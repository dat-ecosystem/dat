...
dat.init = function(options, cb) {
    ...
    self._mkdir(options, function (err) {
      ...
      updateDatJson(function(err, json) {
        ...
        writeDatJson(json, function(err) {
          if (err) return cb(err)
          if (options.readme === false) return initStorage(cb)

          writeDatReadme(function (err){
            // failing to write a readme shouldn't block creation of the dat.
            if (err) console.log('Failed to create a readme.')
            initStorage(cb)
          })
        })
      })
  ...
  function writeDatReadme(cb) {
    var text = options.description ? options.description + '\n' : ''
    fs.writeFile('README.md', options.name + '\n----------\n' + text , cb)
  }

...
dat.printProgress = function(readable, writeable, opts) {
  ...
  if (opts.quiet) {
    ...
    var progPrint = progressPrinter(opts)
    readable.pipe(progPrint)
    if (writeable) progPrint.pipe(writeable)
    else progPrint.resume()
  }
}
...
function progressPrinter(opts) {
  ...
  function logProg(runtime, transferred) {
    if (runtime === 0) return clearLog(transferred + ' rows ' + verb + ' in less than a second\n')

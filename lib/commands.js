...
dat.createWriteStream = function(options) {
  ...
  if (options.csv || options.f == 'csv') {
    ...
    function onFirstCSV(buf) {
      ...
      var newColumns = self.meta.getNewColumns(columns, self.meta.json.columns)
      if (newColumns.length > 0) self.meta.addColumns(newColumns, function(err) {
        if (err) console.error('error updating columns', err)
        if (options.primary) primaryIndex = self.meta.json.columns.indexOf(options.primary)
      })
    }
    ...
    function onFirstJSON(obj) {
      var newColumns = self.meta.getNewColumns(Object.keys(JSON.parse(obj)), self.meta.json.columns)

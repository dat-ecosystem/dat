...
dat.clone = function(options, cb) {
  ...
  self.options.remotes.origin = {url:options.remote}
...
dat._level = function(dbPath, opts, cb) {
  ...
  function localDb() {
    return levelup(self.options.leveldownPath || dbPath, {db: self.options.leveldown}, cb)
  }
}
...
dat._storage = function(options, cb) {
  ...
  if (options.skim) {
    backend = skimBlobs(backend, this._getRemote(options.remote || getRemoteUrl(this.options.remotes.origin)))
  }

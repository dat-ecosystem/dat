  ...
  function remoteDat(remote, cb) {
    ...
    function cloned(err) {
      ...
      request({json: true, uri: remote + '/_package'}, function(err, resp, json) {
        if (err) return cb(err)
        self.meta.write(json, function(err) {
          if (err) return cb(err)
          self.meta.json = json
...
dat.level = function(dbPath, opts, cb) {
  ...
  if (opts.server) remoteDb(opts)
  else localDb()

  function remoteDb(opts) {
    console.log('REMOTEDB', opts)
    self.db = multilevel.client(opts.manifest)
    installAddons(self.db)
  }
  
  function localDb() {
    self.db = level(dbPath, dbOpts, cb)
    installAddons(self.db)
  }

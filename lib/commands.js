...
dat.createBlobReadStream = function(key, name, opts) {
  ...
  self.get(key, opts, function(err, doc) {
    ...
    if (!doc.blobs[name]) return proxy.emit('error', new Error('file is not attached to row'))
...
dat._decoder = function() {
  var self = this
  return through.obj(function(data, enc, cb) {
    cb(null, self.schema.decode(data.value, data.key, data.version))
  })
}
...
dat.createVersionStream = function(key, opts) {
  return pump(this.storage.createVersionStream(key, opts), this._decoder())
}
...
dat.createReadStream = function(opts) {
  return pump(this.storage.createReadStream(opts), this._decoder())
}
...
dat.put = function(key, doc, opts, cb) {
  ...
  var afterMerge = function(err) {
    if (err) return cb(err)
    self.storage.put(key, isBuffer ? doc : self.schema.encode(doc), {version:version}, cb)
  }
...
dat.get = function(key, opts, cb) {
  ...
  this.storage.get(key, opts, function(err, value, version) {
    if (err) return cb(err)
    cb(null, self.schema.decode(value, key, version))
  })
}

dat.delete =
dat.del = function(key, opts, cb) {
  this.storage.del(key, opts, db)
}

...
dat.createWriteStream = function(options) {
  ...
  if (headers && !(headers instanceof Array)) headers = [headers]
  ...
  var primaryIndex
  if (options.primary) {
    var currentHeaders = store.meta.columns.length ? store.meta.columns : headers
    if (currentHeaders) primaryIndex = currentHeaders.indexOf(options.primary)
  }
  ...
  if (options.csv || options.f == 'csv') {
    ...
    newlineParser.once('data', function(obj) {

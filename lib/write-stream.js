...
WriteStream.prototype.setPrimaryKeys = function(cb) {
  ...
  var pFunc = options.primaryFunction
  ...
  // Make my funk the pFunc
  if (pFunc) this.primaryFunction = options.primaryFunction
...
WriteStream.prototype.getPrimaryKey = function(row) {
  ...
  for (var i = 0; i < idx.length; i++) {
    ...
    if (prim === '_id' && !pkey) pkey = docUtils.uuid()
    if (this.primaryFunction) pkey = this.primaryFunction(pkey)

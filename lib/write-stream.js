...
WriteStream.prototype.csvParsers = function() {
  ...
  function onFirstCSV(buf, next) {
    ...
    if (self.primary) { self.primaryIndex = csvColumns.indexOf(self.primary) }
    ...
    if (newColumns.length > 0) {
      self.meta.addColumns(newColumns, function(err) {
        ...
        if (self.primary) self.primaryIndex = self.meta.json.columns.indexOf(self.primary)
        next()
      })
    } else {
      next()
...
WriteStream.prototype.setPrimaryKeys = function() {
  ...
  if (this.primary) {
    ...
    if (currentColumns) this.primaryIndex = currentColumns.indexOf(this.primary)
    this.onRow = function (row) {
      var primaryVal
      if (self.primary === '_id' || self.primaryIndex > -1) {
        if (Array.isArray(row)) primaryVal = row[self.primaryIndex]
        else if (bops.is(row)) primaryVal = bufferAt(row, self.primaryIndex)
        else primaryVal = row[self.primary]        
      }
      if (self.primary === '_id' && !primaryVal) primaryVal = store.uuid()
      self.primaryKeys.push(primaryVal)
...
WriteStream.prototype.writeBatch = function (rows) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    if (this.primary) doc._id = this.primaryKeys.shift().toString()
...
WriteStream.prototype.checkRows = function (rows, cb) {
  ...
  for (var i = 0; i < len; i++) {
    var key = this.primaryKeys[i].toString()
    store.get(key, onRow)
  }

...
module.exports = function writeStream(dat, opts) {
  ...
  function writeBatch(structs, enc, cb) {
    ...
    function rowDone(err, key, updated, version) {
      ...
      if (opts.results && !err) self.push(dat.schema.decode(updated, key, version))
      pending--
    ...
    for (var i = 0; i < len; i++) {
      ...
      processor.documents++
      processor.emit('update')
      put(struct.key, struct.value.length === 0 ? new Buffer([0]) : struct.value, {version:struct.version, force:opts.force}, rowDone)
    }
  }
}

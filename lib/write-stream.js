...
WriteStream.prototype.csvParsers = function() {
  ...
  function onFirstCSV(buf, next) {
    ...
    if (newColumns.length > 0) {
      self.meta.json.columns = []
      self.meta.addColumns(csvColumns, function(err) {
...
WriteStream.prototype.setPrimaryKeys = function(cb) {
  ...
  this.primary = options.primary || '_id'
...
WriteStream.prototype.writeBatch = function (rows) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    var keys = docUtils.rowKeys(store.keys, store.sep, meta._id, meta._rev, seq, schema.version)

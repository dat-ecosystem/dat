...
module.exports = function(dat, opts) {
  ...
  pipeline.push(
    ...
    byteStream({ limit: dat.dbOptions.writeBufferSize, time: opts.batchTime }),
  ...
  function writeBatch(structs, enc, cb) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      dat.put(null, struct.data, {meta: struct, overwrite: opts.overwrite, skipSchemaCheck: true }, rowDone)
    }
  }
}

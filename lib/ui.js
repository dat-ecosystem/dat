...
module.exports.swarmMsg = function (dat) {
  var msg = ''
  if (dat.swarm && dat.swarm.peers) msg = 'Connected to ' + dat.swarm.peers + ' peers. '
...
module.exports.keyMsg = function (key) {
  ...
  msg += 'The Share Link is for you to share with others so that they will be able to get the files'
  return msg + '\n'
}

module.exports.onerror = function (err) {
  if (err.message) {
    if (err.message.indexOf('Resource temporarily unavailable') > -1) err = 'There is a dat process already running in this directory.'
  }

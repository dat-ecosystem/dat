  ...
  function getDb (cb) {
    ...
    function tryResume () {
      ...
      db.get('!dat!key', function (err, value) {
        ...
        if (self.key && self.key.toString('hex') !== value) return cb('Another Dat was already downloaded here.')
...
Dat.prototype.download = function (cb) {
  ...
  archive.once('download', function () {
    if (self.stats.bytesTotal === 0) updateTotalStats() // TODO: fix https://github.com/maxogden/dat/issues/502
  })

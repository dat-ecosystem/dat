...
module.exports = function (type, opts, dat) {
  ...
  function start (err, theDat) {
    ...
    opts.end = opts.exit || false
    network = dat.joinNetwork(opts)
    network.once('connection', function (peer) {
  ...
  function checkDone () {
    ...
    if (!archive.metadata.length) return false
    if (!dat.downloaded) return false

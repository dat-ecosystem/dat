...
module.exports = function(dat, remote, cb) {
  fetchMeta(dat, remote, function(err, remoteMeta) {
    ...
    // make sure that when you clone the schema.json you default to 'leveldown-prebuilt' and not a custom backend
...
function hyperClone(dat, remote, cb) {
  ...
  var backup = fbackup.clone(remote + '/api/archive', opts, function(err) {

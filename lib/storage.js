...
function Database (db, meta, cb) {
  ...
  this.errors = {
    conflict: function() {
      var err = new Error('Document update conflict. Existing revision is newer than incoming one')
      err.conflict = true
      return err
    }
  }
...
Database.prototype.createReadStream = function(opts) {
  ...
  function write(row) {
    ...
    self.db.get(key, { valueEncoding: 'binary'}, function(err, val) {
      ...
      if (--pending === 0 && stream.ended) stream.queue(null)
    })
  }
  ...
  function end() {
    ...
    if (pending === 0) stream.queue(null)
  }
...
Database.prototype.createChangesStream = function (opts) {
  ...
  sequences.on('data', function (seqRow) {
    ...
    if (opts.include_data) {
      ...
      self.get(entry.id, getOpts, function (e, value) {
        ...
        if (--pending === 0 && ended) seqStream.queue(null)
      })
    } else {
      seqStream.queue(entry)
    }
  ...
  sequences.on('end', function () {
    ended = true
    if (pending === 0) seqStream.queue(null)
  })

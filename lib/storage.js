...
Database.prototype.put = function (rawDoc, buffer, opts, cb) {
  ...
  function check() {
    ...
    self.get(doc._id, function(err, existing) {
      ...
      if (opts.meta._rev || opts.reviseConflicts) {
        doc = existing
        return store()
      }
  ...
  function store() {
    if (!opts.overwrite || !doc._rev) updated = docUtils.updateRevision(doc, buffer, self.meta.json.columns)
    else updated = doc
...
Database.prototype.createReadStream = function(opts) {
  ...
  if (opts.limit) seqOpts.limit = opts.limit

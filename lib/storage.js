...
Database.prototype.currentData = function() {
  ...
  function getNext(key, cb) {
    ...
    self.mutex.peekFirst(opts, function (e, key, value) {
      ...
      var schema = self.meta.schemas[decoded._ver]
...
Database.prototype.get = function (key, opts, cb) {
  ...
  this.mutex.peekLast(opts, function (e, k, v) {
    if (e) return cb(e)
    var decoded = docUtils.decodeKey(k)
    var schema = self.meta.schemas[decoded._ver]
    v = jsonBuffStream.decode(schema.columns, v)
    var doc = extend(decoded, v)
    delete doc._seq
    delete doc._ver
    cb(null, doc)
  })
...
Database.prototype.createReadStream = function(opts) {
  ...
  function write (data) {
    var decoded = docUtils.decodeKey(data.key)
    var schema = self.meta.schemas[decoded._ver]
    var obj = jsonBuffStream.decode(schema.columns, data.value)
    var doc = extend(decoded, obj)
    delete doc._seq
    delete doc._ver
    this.queue(doc)
  }

...
Database.prototype.currentData = function() {
  ...
  function getNext(key, cb) {
    ...
    self.mutex.peekFirst(opts, function (e, key, value) {
      ...
      var decoded = docUtils.decodeKey(key)
      value = jsonBuffStream.decode(self.meta.json.columns, value)
...
Database.prototype.get = function (key, opts, cb) {
  ...
  this.mutex.peekLast(opts, function (e, k, v) {
    if (e) return cb(e)
    v = jsonBuffStream.decode(self.meta.json.columns, v)
    var doc = extend(docUtils.decodeKey(k), v)
    delete doc._seq
    cb(null, doc)
  })
}
...
Database.prototype.put = function (rawDoc, buffer, opts, cb) {
  ...
  function store() {
    ...
    var keys = docUtils.rowKeys(self.keys, self.sep, updated._id, updated._rev, seq, self.meta.json.schemaVersion)
...
Database.prototype.createReadStream = function(opts) {
  ...
  var decoderStream = through(write)
  ...
  function write (data) {
    var obj = jsonBuffStream.decode(self.meta.json.columns, data.value)
    var doc = extend(docUtils.decodeKey(data.key), obj)
    delete doc._seq
    this.queue(doc)
  }
}

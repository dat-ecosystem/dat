...
Database.prototype.get = function (key, opts, cb) {
  ...
  this.mutex.peekLast(opts, function (e, k, v) {
    if (e) return cb(e)
    if (!isJSON) v = jsonbuff.decode(self.meta.json.columns, v)
...
Database.prototype.put = function (rawDoc, buffer, opts, cb) {
  ...
  var newColumns = this.meta.getNewColumns(columns, this.meta.json.columns)
  ...
  function store() {
    if (!buffer) buffer = jsonbuff.encode(doc, self.meta.json.columns)

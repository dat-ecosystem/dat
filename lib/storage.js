...
Database.prototype.rowKeys = function(id, rev, seq) {
  var seqKey = this._key(this.seqKey, encodeInteger(seq))
  return {
    seq: seqKey,
    row: this._key(this.dataKey, id + this._key(this.revKey, encodeInteger(rev)) + seqKey)
  }
}
...
Database.prototype.del = function (key, cb) {
  this.seq += 1
  var seq = this.seq
  this.mutex.put(this._key(this.seqKey, encodeInteger(seq)), {_deleted: true, _id: key, _seq: seq}, noop)
...
Database.prototype.getSequences = function (opts) {
  ...
  var startKey = this._key(this.seqKey, encodeInteger(since))
  ...
  sequences.on('data', function (change) {
    ...
    if (opts.include_data) {
      ...
      var seqKey = self._key(self.seqKey, encodeInteger(seq))
...
Database.prototype.compact = function (cb) {
  ...
  sequences.on('data', function (row) {
    ...
    if (id !== _id) {
      ...
      deletes.push(self._key(self.seqKey, encodeInteger(seq)))

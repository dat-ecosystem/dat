...
Database.prototype.createReadStream = function(opts) {
  ...
  if (!curOpts.start) curOpts.start = opts.reverse ? currentEnd : currentBeginning
  if (!curOpts.end) curOpts.end = opts.reverse ? currentBeginning : currentEnd
  ...
  function write(row, enc, next) {
    ...
    self.db.get(key, { valueEncoding: 'binary'}, function(err, val) {
      ...
      if (!decoded.deleted) stream.push(decoded)

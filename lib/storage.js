...
// example key: ÿdÿffe33ee5ÿrÿfc0148ÿsÿ01
...
Database.prototype.rowKeys = function(id, rev, seq) {
  var seqKey = this._key(this.seqKey, lexint.pack(seq, 'hex'))
  return {
    seq: seqKey,
    row: this._key(this.dataKey, id + this._key(this.revKey, lexint.pack(rev, 'hex')) + seqKey)
  }
}
...
Database.prototype.del = function (key, cb) {
  this.seq += 1
  var seq = this.seq
  this.mutex.put(this._key(this.seqKey, lexint.pack(seq, 'hex')), {_deleted: true, _id: key, _seq: seq}, noop)
...
Database.prototype.getSequences = function (opts) {
  ...
  var startKey = this._key(this.seqKey, lexint.pack(since, 'hex'))
  ...
  sequences.on('data', function (change) {
    ...
    if (opts.include_data) {
      ...
      var seqKey = self._key(self.seqKey, lexint.pack(seq, 'hex'))
...
Database.prototype.compact = function (cb) {
  ...
  sequences.on('data', function (row) {
    ...
    if (id !== _id) {
      ...
      deletes.push(self._key(self.seqKey, lexint.pack(seq, 'hex')))

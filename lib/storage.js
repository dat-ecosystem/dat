...
Database.prototype.del = function (key, cb) {
  this.seq += 1
  var seq = this.seq
  this.mutex.put(this._key(this.seqKey, lexint(seq, 'hex')), {_deleted: true, _id: key, _seq: seq}, noop)
...
Database.prototype.getSequences = function (opts) {
  ...
  var startKey = this._key(this.seqKey, lexint(since, 'hex'))
...
Database.prototype.compact = function (cb) {
  ...
  sequences.on('data', function (row) {
    ...
    if (id !== _id) {
      ...
      deletes.push(self._key(self.seqKey, lexint(seq, 'hex')))

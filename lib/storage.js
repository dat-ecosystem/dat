...
Database.prototype.get = function (key, opts, cb) {
  ...
  var revKey = ''
  if (opts.rev) revKey = self._key(this.keys.rev, lexint.pack(opts.rev, 'hex'))
...
Database.prototype.put = function (rawDoc, buffer, opts, cb) {
  ...
  if (!opts.overwrite) doc = this.updateRevision(doc)
  ...
  function store() {
    ...
    // console.log('buffer', buffer, doc, self.meta)

...
RestHandler.prototype.createRoutes = function() {
  ...
  router.addRoute("/api/blobs/:hash", this.blobs.bind(this))
...
RestHandler.prototype.blobs = function(req, res, opts) {
  ...
  if (req.method === 'HEAD') {
    var key = opts.key
    var blob = self.dat.blobs.backend.exists(opts.hash, function(err, exists) {
  ...
  if (req.method === 'GET') {
    res.statusCode = 200
    return self.dat.blobs.backend.createReadStream(opts.hash).pipe(res)
  }
  self.error(res, 405, {error: 'method not supported'})
}
...
RestHandler.prototype.stats = function(req, res) {
  ...
  statsStream.pipe(serializer).pipe(res)
}

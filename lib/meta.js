...
Meta.prototype.pullSchema = function(remote, cb) {
  var self = this
  request({ json: true, url: remote + '/api/metadata' }, function(err, resp, json) {
    if (err || resp.statusCode > 299) {
      return cb(err || new Error(resp.statusCode), "Remote schema request failed")
    }

    var existing = self.dat.schema.toJSON()
    ...
    self.dat.schema.merge(incoming.slice(existing.length), cb)
  })
}

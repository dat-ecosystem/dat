...
function clone (opts) {
  ...
  neat.use(function (state, bus) {
    ...
    opts.exit = true
    opts.errorIfExists = true // TODO: do we always want to force this?

    try {
      // validates + removes dat://
      opts.key = stringKey(opts.key)
      createDir(opts.key, ready)
    } catch (e) {
      var re = /[a-zA-Z0-9-.]\/+[a-zA-Z0-9-]+\/[a-zA-Z0-9-]+$/ // match site.com/username/dataset
      if (re.test(opts.key.trim())) return lookup()
      if (e.message.indexOf('Invalid key') === -1) return bus.emit('exit:error', e)
      return bus.emit('exit:warn', 'Link is not a valid Dat link.')
    }
    ...
    function lookup () {
      ...
      nets({ url: url, json: true }, function (err, resp, body) {
        ...
        if (resp.statusCode !== 200) return bus.emit('exit:warn', body.message)
        try {
          opts.key = stringKey(body.url)
          debug('Received key from registry:', opts.key)
          if (opts.key) return createDir(url.split('/').pop(), ready) // dirname is name of repo
        } catch (e) {
          console.error(new Error(e))
        }
        bus.emit('exit:warn', 'Error getting key from registry')
      })
    }
